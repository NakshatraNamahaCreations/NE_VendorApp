...
export default function Home() {
  ...
  const fetchOrders = async () => {
    try {
      const orderRes = await axios.get(
        `${apiUrl.BASEURL}${apiUrl.GET_PRODUCT_BOOKED_ORDERS}${vendor?._id}`,
      );
      if (orderRes.status === 200) {
        setRecentOrder(orderRes.data.reverse());
      }
    } catch (error) {
  ...
  const productData = recentOrders.flatMap(item => item.product_data);
  console.log('productData', productData);

  // Current date
  const currentDate = new Date();

  // Define first and last dates for filtering
  ...
  const totalEarnings = productData.reduce(
    (acc, item) => acc + item.totalPrice,
    0,
  );

  // Calculate This Month's Earnings
  const thisMonthEarnings = productData
    .filter(item => {
      const orderDate = new Date(item.ordered_date);
      return orderDate >= startOfThisMonth && orderDate <= currentDate;
    })
    .reduce((acc, item) => acc + item.totalPrice, 0);

  // Calculate Last Month's Earnings
  const lastMonthEarnings = productData
    .filter(item => {
      const orderDate = new Date(item.ordered_date);
    ...
    <View style={{backgroundColor: 'white', height: '100%'}}>
      ...
      <ScrollView
        ...
        <View style={{paddingHorizontal: 10, marginBottom: 20}}>
          {/* <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
            <View
              ...
              <Text
                ...
                ₹ {parseInt(thisMonthEarnings).toFixed(2)}
              </Text>
            </View>
            <View
              style={{
              ...
              <Text
                ...
                ₹ {parseInt(lastMonthEarnings).toFixed(2)}
              </Text>
            </View>
          </View> */}
              ...
              <View
                ...
                <View style={{marginTop: 10}}>
                  <Text
                    ...
                    ₹ {parseInt(totalEarnings).toFixed(2)}
                  </Text>
                </View>
              </View>
              <Image
                source={require('../../../assets/codifyformatter.png')}
                style={{
                  width: 100, // Adjusted width to account for margins
                  height: 100,
                  borderRadius: 15,
                  resizeMode: 'contain',
                  opacity: 0.4,
          ...
          {/* </View> */}
          {/* uncammand the below recent orders */}
                ...
                <View
                  ...
                  <View
                    ...
                    <Text
                      ...
                      {moment(ele.ordered_date).format('lll')}
                    </Text>
                    <Text
                      style={{
                        fontFamily: 'Montserrat-SemiBold',
          ...
          <View style={{marginBottom: 20}}> 
            {faqList.map(item => (
              <Accordion key={item._id} title={item.question}>
                <Text
                  style={{
                    color: 'black',
                    fontSize: 14,
