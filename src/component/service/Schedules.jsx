...
const Schedules = ({ vendorData }) => {
  ...
  // const vendorId = route.params.vendorData;
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const baseURL = `${apiUrl.BASEURL}${apiUrl.GET_SERVICE_BOOKED_ORDERS}${vendorId._id}`;
      const url =
        eventStatus === 'All'
          ? baseURL
  ...
  console.log('orderedProducts in BookingList>>>>>>>', orderedProducts);

  // const filterData = () => {
  //   if (eventStatus === 'All') {
  //     return orderedProducts;
  //   } else {
  //     return orderedProducts.filter(item => item.order_status === eventStatus);
  //   }
  // };

  // const filterResult = filterData();
  // console.log('result in BookingList>>>>>>>', filterResult.length);

  // const calculatePayout = productData => {
  //   const orderAmount = productData.reduce(
  //     (acc, value) => acc + value.totalPrice,
  //     0,
  //   );
  //   const commissionPercentage =
  //     orderAmount * (vendorId.commission_percentage / 100);
  //   const commissionTax =
  //     commissionPercentage * (vendorId.commission_tax / 100);
  ...
  // const handleSummaryOpen = (productData, id) => {
  //   if (openSummaryId === id) {
  //     // If the clicked item's summary is already open, close it
  //     setOpenSummaryId(null);
  //     setViewSummaryOpen({});
  //   } else {
  //     setOpenSummaryId(!openSummaryId);
  //     const payoutValues = calculatePayout(productData);
  //     setOpenSummaryId(id);
  //     setViewSummaryOpen({
  //       ...payoutValues,
  //     });
            ...
            {orderedProducts.length === 0 ? (
              ...
              <>
                {orderedProducts.map(ele => {
                  const mergeBookingId = orderedProducts.flatMap(service =>
                    service.service_data.map(data => ({
                  ...
                  // console.log('ele', ele.service_data);
                  return (
                    <TouchableOpacity
                      key={ele._id}
                      style={{
                      ...
                      <View>
                        ...
                        <Text
                          ...
                          {ele.service_data?.length} item
                          {ele.service_data?.length > 1 ? 's' : ''} have been{' '}
                          {deliveryStatus?.order_status === 'Order Delivered'
                            ? 'delivered'
                            : 'placed'}
                        ...
                        {/* <View
                          ...
                          <Text
                            ...
                            {ele.eventStartDate} - {ele.eventEndDate}
                          </Text>
                        </View> */}
                        {/* <Text
                          style={{
