...
const Schedules = ({vendorData}) => {
  ...
  const fetchData = async () => {
    try {
      let res = await axios.get(
        `${apiUrl.BASEURL}${apiUrl.GET_SERVICE_BOOKED_ORDERS}${vendorData._id}`,
      );
      if (res.status === 200) {
        setServiceList(res.data.reverse());
      }
    } catch (error) {
  ...
  const findSelectedTask = serviceList.filter(
    ele => moment(ele.orderDate).format('DD/MM/YYYY') === formattedDate,
  );
