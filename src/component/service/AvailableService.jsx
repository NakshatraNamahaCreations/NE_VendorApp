...
import Stepper from '../product/Stepper';
import THEMECOLOR from '../../utilities/color';
import { useRoute } from '@react-navigation/native';

const AvailableService = ({ vendorData }) => {
  useBackHandler();
  ...
  const fetchAllServices = async () => {
    ...
    try {
      let res = await axios.get(
        `${apiUrl.BASEURL}${apiUrl.GET_ALL_SERVICES_BY_VENDOR_ID}${vendorId._id}`,
      );
      if (res.status === 200) {
        const serviceItems = Array.isArray(res.data.service)
          ? res.data.service
          : [];
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    Promise.all([fetchAllServices()]).finally(() => {
      setRefreshing(false);
    });
  }, []);

  const approvedItems = serviceList.filter(
    item => item.approval_status === 'Approved',
  );
  const underReviewdItems = serviceList.filter(
  ...
  const updateProductAvailability = async () => {
    ...
    setLoading(true);
    try {
      const serviceIds = Array.isArray(selectedIds) ? selectedIds : [selectedIds];
      const res = await axios.post(
        `${apiUrl.BASEURL}${apiUrl.UPDATE_SERVICE_AVAILABILITY}`,
        {
                    ...
                    <View
                      ...
                      <View>
                        <Checkbox
                          ...
                          onPress={() => {
                            const updatedProducts = serviceList.map(product =>
                              product._id === item._id
                                ? { ...product, isSelected: !product.isSelected }
                                : product,
                            );
                            setServiceList(updatedProducts);
                          }}
